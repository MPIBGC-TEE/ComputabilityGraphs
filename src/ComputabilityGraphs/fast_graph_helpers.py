...
def combine(g1: FastGraph, g2: FastGraph):
    ...
    for (src, target, edge_dict) in g2.dg.edges(data=True):
        if (
            type(src) == Node
            and type(target) == Decomposition
            and css_key in edge_dict.keys()
        ):
            g.connect_Node_2_Decomposition(
                node=src, target_decomp=target, computer_sets=edge_dict[css_key]
            )
        else:  # if type(src)==Decomposition and type(target)==Node:
            g.connect_Decomposition_2_Node(decomp=src, target_node=target)

    return g


def src_node_computersets_tuples_from_decomp(
    g: FastGraph, d: Decomposition
...
def add_combi_arg_set_graph(
    g: FastGraph, root: Node, decomp: Decomposition, computer_combi: Set[Computer]
) -> Tuple[FastGraph, FrozenSet[Node]]:
    _, passive = decomp
    args = reduce(
        frozenset.union,
        [h.arg_set(computer) for computer in computer_combi],
        frozenset(),
    )
    arg_node = frozenset.union(args, passive)
    avoid_nodes = g.visited_Nodes_on_paths(src=decomp, root=root)
    if any([arg_node.issuperset(avn) for avn in avoid_nodes]):
        ...
        if G.has_edge(*e):
            ...
            G.get_edge_data(*e)[key] = frozenset.union(
                data, frozenset({computer_combi})
            )
            return (G, frozenset())
        else:
            G.add_Node(arg_node)
...
def add_combis_arg_set_graphs_to_decomp(
    ...
    def f(acc, el):
        g, new_nodes = acc
        computer_combi = el
        g_new, combi_new_nodes = add_combi_arg_set_graph(
            g, root, decomp, computer_combi
        )
...
def add_all_arg_set_graphs_to_decomp(
    g: FastGraph, root: Node, decomp: Decomposition, all_computers
) -> Tuple[nx.DiGraph, FrozenSet[Node]]:
    g_new = deepcopy(g)
    ...
    all_combies = h.all_computer_combis_for_mvar_set(
        var_set=active, all_computers=all_computers, avoid_nodes=avoid_nodes
    )
    ...
    return add_combis_arg_set_graphs_to_decomp(g_new, root, decomp, relevant_combis)
...
def prospective_decomp_list_2(node: Node, passive: FrozenSet[Node]):
    # instead of uncomputable variables we use a more general set of
    # uncomputable Nodes
    allowed_actives = [
        a
        for a in h.power_set(node)
        if not any([a.issuperset(p) for p in passive]) and a != Node([])
    ]
...
def add_all_decompositions_to_node(
    ...
    new_decompositions = frozenset(
        filter(lambda d: not (g.has_Decomp(d)), decompositions_2)
    )
...
def add_all_decompositions_to_all_nodes(
    g: FastGraph, root: Node, nodes: FrozenSet[Node], uncomputable: FrozenSet[type]
) -> Tuple[FastGraph, FrozenSet[Decomposition]]:
    g_new = deepcopy(g)
    ...
    def f(acc, node):
        ...
        res_p = g.passive_Nodes_in_Decompositions_on_paths(src=node, root=root)
        passive = frozenset.union(
            frozenset([n for n in res_p if n != Node({})]),
            frozenset([Node({v}) for v in uncomputable]),
        )

        g_new, node_decomps = add_all_decompositions_to_node(g, node, passive=passive)
        new_decompositions = frozenset.union(decompositions, node_decomps)
...
def fast_graph(root_type: type, cs: ComputerSet, given: FrozenSet[type]) -> FastGraph:
    g, new_nodes = last(update_generator(root_type, cs, max_it=30, given=given))
    return g
...
def update_generator(
    root_type: type, cs: ComputerSet, max_it: int, given: FrozenSet[type] = frozenset()
) -> "generator":

    if max_it < 0:
        raise IndexError("update sequence indices have to be larger than 0")
    uncomputable = frozenset.union(h.uncomputable(cs), given)
    print("uncomputable=" + h.node_2_string(uncomputable))

    g, ds = initial_fast_graph(root_type=root_type, cs=cs)
    yield (g, ds)
    counter = 1
    ...
    while True:
        ...
        g, ds = add_all_decompositions_to_all_nodes(
            g, Node({root_type}), ns, uncomputable=uncomputable
        )
...
def initial_fast_graph(root_type: type, cs: ComputerSet) -> FastGraph:
    root = Node({root_type})
    g = FastGraph()
    g.add_Node(root)
    new_decompositions = (
        frozenset([Decomposition(active=root, passive=Node([]))])
        if root_type not in h.uncomputable(cs)
        else frozenset()
    )
...
def add_passive(sg, p):
    ...
    for (s, t, css) in n2des:
        sp = s.add_passive(p)
        tp = t.add_passive(p)
        tsg.add_connected_Node(node=sp, target_decomp=tp, computer_sets=css)
...
def draw_update_sequence(
    ...
    lg = [g for (g, new) in update_generator(root_type, computers, max_it, given)]
    print(type(lg[-1]))
    nr = len(lg)
    fig.set_size_inches(20, 20 * nr)
    # pos = nx.spring_layout(lg[-1].dg)
    # layout alternatives
    pos = nx.spring_layout(lg[-1].dg, scale=10, fixed=None, dim=2)
    ...
    for i in range(nr):
        lg[i].draw_matplotlib(axs[i], mvar_aliases, computer_aliases, pos=pos)
